  ...
  fun(){
    if(this.state.loading){
      return <img src={loading} style={{paddingLeft:'50%'}}/>
        ...
        <h1 style={listyle}><i>Character - {this.props.item.name}</i></h1>
        <hr style={listyle}/><br style={listyle}/>
        <span><img src={this.props.item.image} style={imgs} alt="img"/></span>
        <p style={pstyle}><b>Species : </b>{this.props.item.species}<br/><br/><br/>
        <b>Status : </b>{this.props.item.status}<br/><br/><br/>
        <b>Gender : </b>{this.props.item.gender}<br/><br/><br/>
        <b>Origin : </b>{this.props.item.origin.name}<br/><br/><br/>
        <b>Last Location : </b>{this.props.item.location.name}<br/></p>
...
async componentDidMount(){
  var arr=[];
  for(var i=0;i<this.props.item.episode.length;i++){
  const res=await fetch(this.props.item.episode[i]);
